...
template newSMatrix(matrix, ord) =
  new matrix.data
  matrix.order = ord

template newDMatrix(matrix, Mval, Nval, ord: untyped, A: typedesc) =
  matrix.data = newSeq[A](Mval * Nval)
  matrix.M = Mval
  matrix.N = Nval
  matrix.order = ord
